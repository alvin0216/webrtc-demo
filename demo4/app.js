...
app._io.on('connection', socket => {
  ...
  socket.on('join', ({ account, roomId }) => {
    socket.join(roomId, () => {
      ...
      const userList = [...userMap.values()].map(d => ({ socketId: d.socketId, account: d.account }))

      app._io.in(roomId).emit('joined', userList, { socketId: socket.id, account }) // 发给房间内所有人
    })
  })
...
app._io.on('disconnect', socket => {
  const target = [...userMap.values()].find(item => item.socketId === socket.id)
