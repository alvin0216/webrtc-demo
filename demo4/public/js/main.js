window.onload = function () {
  ...
  */
  ...
  socket.on('apply', ({ from, to }) => {
    if (isCall) { // 收到呼叫 但是当前正在通话中
      reply(from, 3)
    } else {
      if (comfirmConnect(from)) {
        // B 同意创建 
        createP2P({ from, to })
        console.log(2, `B 收到 A 的呼叫，B 现在要回复 A 同意链接 >> 调用 initPeer 创建 perrB, 并且等待 A 的 offer`)
        reply(from, 1)
      } else {
        reply(from, 2)
      }
  ...
  */
  ...
  socket.on('1v1ICE', data => {
    // 接收到 ICE
    try {
      console.log(sessionStorage.account === data.from ? 'A' : 'B', '接收到 1v1 ICE')
  ...
  async function createOffer({ from, to }) {
    try {
      console.log(4, 'peerA 创建成功，peerA 生成 offer，并触发 1v1offer 传送给 B')
      var offerOption = {
